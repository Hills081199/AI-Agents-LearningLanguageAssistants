...
from optimization import apply_optimization_config, performance_metrics, adaptive_optimization, estimate_tokens
...
def generate_vocab_html(vocabulary: List[Dict], has_romanization: bool) -> str:
...
def generate_grammar_html(grammar: List[Dict]) -> str:
...
def generate_lesson_parallel(topic=None, level="HSK 5", language="chinese"):
    """Generate lesson using parallel processing where possible."""
    agents = LanguageLearningAgents(language=language)
    tasks = LanguageLearningTasks(language=language)
    
    config = get_language_config(language)
    
    # Create agents with optimization
    planner = apply_optimization_config(agents.lesson_planner_agent(), 'lesson_planner')
    writer = apply_optimization_config(agents.content_writer_agent(), 'content_writer')
    linguist = apply_optimization_config(agents.linguist_agent(), 'linguist')
    examiner = apply_optimization_config(agents.examiner_agent(), 'examiner')
    writing_assessor = apply_optimization_config(agents.writing_assessor_agent(), 'writing_assessor')
    ...
    # Step 3: Execute content generation, analysis, and quiz in parallel where possible
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Submit all tasks
        future_to_task = {
            executor.submit(execute_single_task, agent, task): task 
            for agent, task in parallel_tasks
        }
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                ...
                task_type = str(task).split()[0].lower()  # Extract task type from description
                results[task_type] = result
            except Exception as exc:
                print(f'Task {task} generated an exception: {exc}')
    
    # Step 4: Generate writing prompt (can be done in parallel with others, but kept separate for clarity)
    ...
    # Parse results
    story_output = results.get('write', '')
    analyze_output = results.get('analyze', '')
    quiz_output = results.get('quiz', '')
    ...
    estimated_tokens = estimate_tokens(str(lesson_data))
    performance_metrics.record_request(duration, estimated_tokens, success=True)
    
    print(f"⚡ Lesson generated in {duration:.2f} seconds")
    print(f"📊 Performance: {performance_metrics.get_metrics()}")
    
    return final_topic, final_markdown, html_content, lesson_data

def execute_single_task(agent: Agent, task: Task) -> str:
    """Execute a single task with its own crew."""
...
def generate_lesson_content(topic=None, level="HSK 5", language="chinese"):
    ...
    return generate_lesson_parallel(topic, level, language)
