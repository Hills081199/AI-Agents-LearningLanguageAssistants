...
export default function WritingExercise({ topic, level, language, apiBaseUrl, initialPrompt, lessonFilename, initialHistory }: WritingExerciseProps) {
    ...
    // If initialPrompt changes (e.g. new lesson generated), update state
    // This might need useEffect if component is not unmounted
    // implementing basic effect for now

    // Note: Since we key the component in parent or remount, just initial state is fine?
    // Let's add useEffect to be safe if parent keeps it mounted.
    // Actually, usually simpler to use key in parent. I will rely on parent re-render or key.
